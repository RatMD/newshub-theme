var NewsHub = (() => {
  ...
  function m(n2) {
    var i2, e2, a2, o2, u2, s2, c2, f2, h2, m2, g2, b2, x2, y2, k2 = 1 / 0, w = [], M = null, T = 0;
    function C(n3) {
      _(T + n3);
    }
    function E(n3) {
    ...
    function H(t2) {
      var r2 = t2 ? null : function() {
        if (s2) {
          var n3 = S(), t3 = n3 ? (T % e2 + e2) % e2 : T, i3 = (n3 ? T % e2 : T) - u2[0][2], r3 = 0 - (i3 < 0 && n3 ? e2 - Math.abs(i3) : i3), c3 = 0, d2 = z(T), f3 = d2.abs, p2 = d2.rel, v2 = u2[p2][2], k3 = u2.map(function(t4, i4) {
            var a3 = r3 + c3;
          ...
          return f3 = L(f3), p2 = O(f3), { abs: L(f3), length: a2, max: y2, maxIdx: m2, min: x2, minIdx: h2, position: T, progress: n3 ? t3 / e2 : T / a2, rel: p2, slides: k3, slidesLength: e2 };
        }
      }();
    ...
    return i2 = { absToRel: O, add: C, details: null, distToIdx: E, idxToDist: I, init: function(t2) {
      ...
      !function() {
        ...
        g2 = h2 = i4 ? p(i4, "min", -1 / 0) : 0, b2 = m2 = i4 ? p(i4, "max", k2) : f2;
        var e3 = p(t3, "min", null), r2 = p(t3, "max", null);
        e3 && (h2 = e3), r2 && (m2 = r2), x2 = h2 === -1 / 0 ? h2 : n2.track.idxToDist(h2 || 0, true, 0), y2 = m2 === k2 ? m2 : I(m2, true, 0), null === r2 && (b2 = m2), p(t3, "align", false) && m2 !== k2 && 0 === u2[O(m2)][2] && (y2 -= 1 - u2[O(m2)][0], m2 = E(y2 - T)), x2 = v(x2), y2 = v(y2);
  ...
  function g(n2) {
    ...
    function p2() {
      m2();
      var t3 = "free-snap" === n2.options.mode, i3 = n2.track, o3 = i3.velocity();
      e2 = l(o3);
      var u3 = n2.track.details, d2 = [];
      if (o3 || !t3) {
        var p3 = v2(o3), h3 = p3.dist, g2 = p3.dur;
        if (g2 = s2(g2), h3 *= e2, t3) {
          var b2 = i3.idxToDist(i3.distToIdx(h3), true);
          b2 && (h3 = b2);
        }
        d2.push({ distance: h3, duration: g2, easing: f2 });
        var x2 = u3.position, y2 = x2 + h3;
        if (y2 < r2 || y2 > a2) {
          var k2 = y2 < r2 ? r2 - x2 : a2 - x2, w = 0, M = o3;
          if (l(k2) === e2) {
            var T = Math.min(Math.abs(k2) / Math.abs(h3), 1), C = function(n3) {
              return 1 - Math.pow(1 - n3, 1 / 3);
            }(T) * g2;
            d2[0].earlyExit = C, M = o3 * (1 - T);
          } else
            d2[0].earlyExit = 0, w += k2;
          var E = v2(M, 100), z = E.dist * e2;
          n2.options.rubberband && (d2.push({ distance: z, duration: s2(E.dur), easing: f2 }), d2.push({ distance: -z + w, duration: 500, easing: f2 }));
        }
        n2.animator.start(d2);
      } else
        n2.moveToIdx(c2(u3.abs), true, { duration: 500, easing: function(n3) {
    ...
    function h2() {
      ...
      t3 && (r2 = t3.min, a2 = t3.max, o2 = t3.minIdx, u2 = t3.maxIdx);
    }
    function m2() {
      n2.animator.stop();
    }
    n2.on("updated", h2), n2.on("optionsChanged", h2), n2.on("created", h2), n2.on("dragStarted", function() {
      m2(), t2 = i2 = n2.track.details.abs;
    }), n2.on("dragEnded", function() {
      var e3 = n2.options.mode;
      "snap" === e3 && function() {
        var e4 = n2.track, o3 = n2.track.details, u3 = o3.position, s3 = l(e4.velocity());
        (u3 > a2 || u3 < r2) && (s3 = 0);
        var d2 = t2 + s3;
        0 === o3.slides[e4.absToRel(d2)].portion && (d2 -= s3), t2 !== i2 && (d2 = i2), l(e4.idxToDist(d2, true)) !== s3 && (d2 += s3), d2 = c2(d2);
        var f3 = e4.idxToDist(d2, true);
        n2.animator.start([{ distance: f3, duration: 500, easing: function(n3) {
          return 1 + --n3 * n3 * n3 * n3 * n3;
        } }]);
      }(), "free" !== e3 && "free-snap" !== e3 || p2();
    }), n2.on("dragged", function() {
      i2 = n2.track.details.abs;
    });
  }
  function b(n2) {
    var t2, i2, e2, r2, a2, f2, p2, v2, h2, m2, g2, b2, x2, y2, k2, w, M, T, C = c();
    function E(t3) {
      if (f2 && v2 === t3.id) {
        var o2 = D(t3);
        if (h2) {
    ...
    function A(n3) {
      var t3 = L(), i3 = t3 ? n3.y : n3.x, e3 = t3 ? n3.x : n3.y, r3 = void 0 !== x2 && void 0 !== y2 && Math.abs(y2 - e3) <= Math.abs(x2 - i3);
      return x2 = i3, y2 = e3, r3;
    }
    function D(n3) {
      return L() ? n3.y : n3.x;
    }
      ...
      if (C.purge(), n2.options.drag && !n2.options.disabled) {
        ...
        i3 = n2.options.dragSpeed || 1, p2 = "function" == typeof i3 ? i3 : function(n3) {
          ...
          o("[".concat(n3, "]:not([").concat(n3, "=false])"), t2).map(function(n4) {
            C.add(n4, "mousedown", s), C.add(n4, "touchstart", s);
          });
        ...
        o("[".concat(r3, "]:not([").concat(r3, "=false])"), n2.container).map(function(n3) {
          return function(n4) {
            ...
            C.input(n4, "touchstart", function(n5) {
              ...
              if (t3 = D(i4), (a3 < 0 && o2 > 0 || a3 > 0 && o2 < r4) && k2 && s2)
                return w = true;
              k2 = false, u(i4), w = false;
  ...
  function y(t2, i2) {
    return function(e2) {
      ...
      function T(n2, t3) {
        if (n2)
          return k2(n2), void M(t3);
        I(), A();
        var i3 = y2();
        C(), y2() !== i3 ? M(t3) : w(t3), e2.emit("updated");
      }
      function C() {
  ...
  var k = function(n2, i2, e2) {
    try {
      ...
      }(i2, t([y(n2, { drag: true, mode: "snap", renderMode: "precision", rubberband: true, selector: ".keen-slider__slide" }), x, b, g], e2 || [], true));
    } catch (n3) {
      console.error(n3);
    }
  };
  ...
  var _KeenSliderNewsHub = class {
    ...
    constructor(element, options = {}) {
      ...
      this.slider = new k(this.root, keenSliderOptions, keenSliderPlugins);
    }
    slideTo(direction) {
      if (direction === "next") {
        this.slider.next();
    ...
    pluginIndicators(slider) {
      ...
      if (typeof this.config.indicators === "boolean") {
        ...
        let indicators = query_default(this.config.indicators, this.root).filter((el) => {
      ...
      const updateControls = ((slider2) => {
        ...
        controls.map((control) => {
          if (slider2.options.loop) {
            ...
            let per = slider2.options.slides.perView || 1;
